...
週次で S3 に蓄積された資産情報を取得し、運用指標を計算してサマリを通知します。

- 毎週日曜 09:00 (JST) に自動実行される
- S3 から最新の資産情報 JSON を取得する（`assets/{YYYY}/{MM}/{DD}.json`）
- 総評価（拠出金額累計、評価損益、資産評価額）を通知する
- 運用指標（運用年数、運用利回り、目標利回り、想定受取額）を計算・通知する
- 商品別の資産情報を通知する
- LINE Messaging API 経由で通知を送信する
- S3 からのデータ取得失敗時は ERROR ログを出力する
- 通知送信失敗時は ERROR ログを出力し、Lambda リトライを行う
...
- 各 Lambda 関数のエラーメトリクスを CloudWatch Alarm で監視する
- エラー発生時、SNS Topic 経由で通知する
