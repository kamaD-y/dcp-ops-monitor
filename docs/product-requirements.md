...
DCP Ops Monitor は、確定拠出年金（Defined Contribution Plan）の運用状況を自動で監視・通知するシステムです。週次で運用指標を取得し、サマリとして通知することで、運用管理負荷を軽減します。

### 目的

- **運用状況の可視化**: 確定拠出年金の運用指標を定期的に取得し、わかりやすい形式で通知
...
週次で確定拠出年金の運用状況を取得し、通知します。

- 毎週月曜日 09:00 (JST) に自動実行される
- 認証を行い、資産評価情報を取得する
- 総評価額（拠出金額累計、評価損益、資産評価額）を取得する
- 商品別の資産情報を取得する
- 総評価額と商品別資産を含むメッセージを送信する
- 金額は 3 桁区切りでフォーマットする
- 通知失敗時はリトライする
